package lawsof.physics.lib.objects.block.render.model
 ...
 */
class CustomModelBase extends ModelBase {
  initShapes

  def texSize: (Int, Int)

  def render(p: Entity, f1: Float, f2: Float, f3: Float, f4: Float, f5: Float, f6: Float) = {
  ...
  def initCubeShape(offset: TextureOffset = new TextureOffset(0, 0),
                    ...
                    rotation: (Float, Float, Float) = (0F, 0F, 0F),
