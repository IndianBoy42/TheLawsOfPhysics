...
abstract class MetalBase(val metal: MetalInfo) {
  ...
  def createBlock(prefix: String, mat: Material) = {
    addBlock(new BlockBase(s"${prefix}_${metal.name}", mat, CTabs.metalsTab, metal.getBlockInfo), prefix)
  }
  ...
  def createArmor(prefix: String, mat: ItemStack) = {
    ...
    getArmor(prefix).createArmorSet(CTabs.metalsTab)
  }
  ...
  def createTools(prefix: String, mat: ItemStack) = {
    ...
    getTool(prefix).createToolSet(CTabs.metalsTab)
  }
